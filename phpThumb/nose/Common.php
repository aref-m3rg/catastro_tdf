...
//Connection Settings @0-4BE840BC
$CCConnectionSettings = array (
    "catastro" => array(
        "Type" => "MySQL",
        "DBLib" => "MySQL",
        "Database" => "catastro_tdf_nuevo",
        "Host" => "localhost",
        "Port" => "",
        "User" => "php_user",
        "Password" => "phpuser",
...
//CCLoginUser @0-ED129CB6
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBcatastro();
    $SQL = "SELECT usuarios_id, us_grup_id, usuarios_clave FROM usuarios WHERE usuarios_login=" . $db->ToSQL($login, ccsText) . " AND usuarios_clave=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("usuarios_id"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("us_grup_id"));
    }
    return $Result;
}
//End CCLoginUser
