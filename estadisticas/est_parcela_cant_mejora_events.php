...
// -------------------------
		...
		$col = array (
		  'tipo_est_parc_descr'=>'Estado Parc.',
		  ...
		  'mejora_sup_cub'=>'Cubierta',
		  'mejora_sup_semi_cub'=>'Semi_Cubierta',
		  'mejora_sup_cub_2'=>'Cubierta_negocio',
		  'mejora_anio_construccion'=>'Año Construcc.',
		  'mejora_f_alta'=>'Fecha DDJJ',
		  'tipo_mejora_decla_descrip'=>'Registracion');

		CCSetSession('qryXls',$q);
		CCSetSession('colXls',$col);
		CCSetSession('nameXls',"Consulta de Parcelas mejoras");
/*
		...
		$select="SELECT parcelas.parcela_super_mensura, parcelas.parcela_sup_uf, parcelas.parcela_val_ampliac, parcelas.parcela_val_mejora,
		 parcelas.parcela_val_tierra, parcelas.parcela_val_total, parcelas.unidades_medidas_id, unidades_medidas.unidades_medidas_metros";
		$from="FROM parcelas 
...
// -------------------------
		$SELECT = "SELECT IF( plano_nro, CONCAT('T.F. ', CONCAT_WS('-', planos.tipo_depto_parc_id, CONCAT(tipo_plano_abrev, plano_nro ), RIGHT( plano_anio, 2 ) ) ), IFNULL( tmp_plano, 'Sin Mensura') ) AS plano,
					...
					tipos_deptos_parcela.tipo_depto_parc_id AS tipos_deptos_parcela_tipo_depto_parc_id, parcelas.parcela_id,
					...
					mejoras.mejora_sup_cub, mejoras.mejora_sup_semi_cub, mejoras.mejora_sup_cub_2, mejoras.mejora_anio_construccion, mejoras.mejora_f_alta, tipos_mejoras_decla.tipo_mejora_decla_descrip,
					personas_parcelas.parcela_id AS personas_parcelas_parcela_id";
		$FROM = "   parcelas
					INNER JOIN personas_parcelas ON parcelas.parcela_id = personas_parcelas.parcela_id
					INNER JOIN personas ON personas_parcelas.persona_id = personas.persona_id
					INNER JOIN mejoras ON parcelas.parcela_id = mejoras.parcela_id
					...
					LEFT JOIN tipos_mejoras_decla ON mejoras.tipo_mejora_decla_id = tipos_mejoras_decla.tipo_mejora_decla_id
					LEFT JOIN tipos_restricc_parcela ON parcelas.tipo_restricc_parcela_id = tipos_restricc_parcela.tipo_restricc_parcela_id
		...
		//echo $Component->ds->Where;
		$Component->ds->Where .= " GROUP BY mejoras.mejora_id ";
...
//Page_BeforeInitialize @1-02560680
function Page_BeforeInitialize(& $sender)
{
    $Page_BeforeInitialize = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $est_parcela_cant_mejora; //Compatibility
